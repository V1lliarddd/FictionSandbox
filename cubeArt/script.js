...
const TILES_Y = 9;
const TILES_SIZE = 60;
const PREVIEW_WIDTH = TILES_X * TILES_SIZE;
const PREVIEW_HEIGHT = TILES_Y * TILES_SIZE;
...
function setTileImage(tile, side, imagePath) {
  ...
  const offsetX = -(tile.col * TILES_SIZE);
  const offsetY = -(tile.row * TILES_SIZE);
...
projectLinks.forEach((link) => {
  link.addEventListener('mouseenter', () => {
    projectLinks.forEach((l) => l.classList.add('active'));
