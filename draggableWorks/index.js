...
const startDrag = (x, y) => {
  isDragging = true;
  ((isClick = true), (clickStartTime = Date.now()));
...
const handleMove = (curX, curY) => {
  if (!isDragging || curX === undefined || curY === undefined) return;

  const deltaX = curX - previousMouse.x;
  const deltaY = curY - previousMouse.y;
  ...
  previousMouse.x = curX;
  previousMouse.y = curY;
};
...
const onPointerUp = (event) => {
  ...
  if (isClick && Date.now() - clickStartTime < 200) {
    ...
    if (endX !== undefined && endY !== undefined) {
      ...
      const texIndex = Math.floor((cellX + cellY * 3) % projects.length);
...
const init = async () => {
  ...
  renderer.setClearColor(
    new THREE.Color(bgColor[0], bgColor[1], bgColor[2], bgColor[3]),
  );
